...
To satisfy this goal, we present the following adversarial loss to support more effective multi-purpose discriminators. Since the model is trained in an end-to-end fashion, we can obtain more consistent information exhibited in both lower and higher-resolution outputs compared with StackGAN (see experiments). }
...
\begin{figure}[t]
	...
	\vspace{-.5cm}
	\caption{Given an image in the side output pyramid of the generator, the corresponding discriminator $D_i$ computes the matching-aware pair loss and adaptive local image loss (outputting a $R{\times}R$ probability map to classify real or fake patches). The focal range increases as image resize grows. Lower range results in finer results. }  \vspace{-.3cm}
	\label{fig:loss}
\end{figure}
