...
%the generated images preserve the object color and sketch described in text and fine-grained descriptive details in image pixels as well. 
...
To guarantee the image diversity and semantic consistency, we enforce discriminators at multiple side outputs of the generator to simultaneously differentiate real-and-fake image-text pairs as well as real-and-fake local image patches. %For more details, please refer to Section~\ref{sec:localloss}%focal 
...
%% It is a regularization to limit the discriminators to use the unexpected information to make the decision. 
...
%We discuss related work and further clarify the novelty of our method by comparing with them.
...
%for example, label smoothing, and discriminator feature matching \cite{salimans2016improved, }, training stronger discriminator is beneficial to encourage generators \cite{arjovsky2017wasserstein}. Balancing through an equilibrium enforcing method \cite{berthelot2017began}
...
%%% Huang \etal \cite{huang2016stacked} propose a top-down stacked GAN to leverage mid-level representations, which shares some similarities with StackGAN and our method. However, this method needs multiple symmetric bottom-up pre-trained discriminators and the usage of high-resolution image is unclear. 
...
%Since our method is trained in an end-to-end manner, the lower-resolution side outputs can fully enjoy the top-down knowledge from higher resolutions, such that we can observe consistent image structures and styles appear in both low and high resolutions.
...
%\item Further forcing the generated samples to have only global but also local similar statistics to real image patches, which is beneficial for rendering fine-grained details. 
%
%\item \textcolor{red}{Image loss without using paired text can be beneficial in the sense that it focuses more on image information. The richness of discriminator supervision usually has advantages for generalization. } 
%\vspace{-.2cm}
%\end{itemize}
...
%with an extra branch of the discriminator for the pair loss (see Section 3.4).
%computes a $R{\times}R$ probability map. 
...
%In practice, the local image adversarial loss can be implemented as a fully CNN \cite{shrivastava2016learning,zhu2017unpaired,isola2016image}. 
...
%Specifically, a larger $R_i$ value results in a smaller local range and finer focus of discriminators.
...
%In addition, at the very beginning of the training,  generated images are usually far away from true image distribution, the discriminator can very easily judge a pair as invalid without using much pair information. \textcolor{red}{need more explanation.}
...
%During the training stage, we randomly pick-up a cropped view of the original images, and pick 4 image sentence embedding and use the mean as conditioning vector.
...
%For example, in the first row, the generated birds gradually turn the color from yellow to gray. In the second row, 
